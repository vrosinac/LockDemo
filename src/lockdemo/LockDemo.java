...
public class LockDemo {
    ...
    public static void main(String[] args) {
       ...
       if (output.isEmpty())
        {
            output = "C:\\JavaProjects\\LockDemo\\test.html";
        }
        ...
        try
        {
            ...
            System.out.println("reading from: " + input + " writingn to: " + output);
	 ...
	 Calendar c = Calendar.getInstance();
         ...
         sout = sout+"<br>" + "(last update: " +dtf.format(now) + " " + stz + ")<br> please reach jamesalexander.johnfuller@finastra.com, piotr.weglarz@finastra.com vincent.rosinach@finastra.com if the page stops refreshing testtt<br><br>"; //2016/11/16 12:08:43
         ...
         for (int i = 0; i < nList.getLength(); i++)
         {
            ...
            if (node.getNodeType() == Node.ELEMENT_NODE)  // otherwise do nothing
            {
                //Print each env detail
                Element eElement = (Element) node;  /// we will need to pass the element down
                // or to extract all its values and pass then down.... pass rather the elt down
                // now we loop rouond the xml elements, one per thread
                // we don't know how many elements there will be  .... so new each time + copy nto thread by value
                // COUNTER TEMP - 1 array of xmlElement  and one array of ProcessElement
                ...
                eltfields.global_password =  eElement.getElementsByTagName("global_password").item(0).getTextContent();
                eltfields.env_name = eElement.getElementsByTagName("environmentName").item(0).getTextContent(); 
                eltfields.machine_name =  eElement.getElementsByTagName("machineName").item(0).getTextContent();
                eltfields.url = eElement.getElementsByTagName("url").item(0).getTextContent(); 
                eltfields.login = eElement.getElementsByTagName("login").item(0).getTextContent();
                ...
                eltfields.zone1_password = eElement.getElementsByTagName("zone1_password").item(0).getTextContent();
                eltfields.zone2_login = eElement.getElementsByTagName("zone2_login").item(0).getTextContent();
                eltfields.zone2_password = eElement.getElementsByTagName("zone2_password").item(0).getTextContent();
          
                /* xmlElement elt1 = new xmlElement("aa", "bb");
                xmlElement elt2 = new xmlElement("cc", "dd");
                xmlElement elt3 = new xmlElement("ee", "ff");*/
               runables[i] = new ProcessElement(eltfields, myHTMLGenerator,i);  // Ihave traces there in the constructor
                 // Creating the thread 
                threads[i] = new Thread(runables[i], " Thread"+i);   // not sure where i can add traces
                //starting thread 
         ...
         while (end == false)
         {
           ...
           // System.out.println("checking termination");
            for (int j=0; j<size; j++)
            {
                ...
                if (threads[j] != null)
                {
                    if(threads[j].getState()==Thread.State.TERMINATED && terminatedStatuses[j]==false )
                    { 
                     ...
                     {
                         //System.out.println("threads[" +j +"] not terminated");
                         end= false;    
                     }
                }
                try{
...
// additioonal columsn for the visitors
    ...
    catch (InterruptedException e)
    {
     ...
     result retval = ConnectAndGetVersion(threadName, database_type, database_url, schema, global_login, global_password);
     if (retval.up)
     {
         sout = sout + "<th align=\"left\" style=\"background-color:lightgreen\" >" + environmentName + "</th>";
     }
     else
     {
...
/////////////////////////////
  ...
  //static protected result ConnectAndGetVersion(String threadName, String dbType, String url, String schema, String user, String password)
   public result ConnectAndGetVersion(String threadName, String dbType, String url, String schema, String user, String password)
    {// we either return a version .... or we fail which means we could not connect
        ...
        try
        {
            ...
            if (dbType.equals("DB2") || dbType.equals("DB2ForTI229") ){
                try
                 {
                    ...
                    System.out.println(threadName + " **** Called Class.forName(\"ibm...");
                    try
                    {
                       sleep(100);
                    }
                ...
                {
                    System.out.println(threadName + " **** Exceptopn in called Class.forName(\"ibm...");
                      e.toString();
                }
           }
            if (dbType.equals("Oracle")|| dbType.equals("OracleForTI25"))
            {
                ...
                try{
                 ...
                 System.out.println(threadName + " **** Called Class.forName(\"oracle...");

                }
                catch (ClassNotFoundException e)
                {
                    System.out.println(threadName + " **** Exceptopn in called Class.forName(\"oracle...");
                    e.toString();
                }
                try
                {
            ...
            //con = DriverManager.getConnection (connectionString, "TIGLOBAL28", "G10b@!123");                 
            
            if (!user.isEmpty() && !password.isEmpty() &&  !connectionString.isEmpty())
            {
               /* 
        ...
        */
                ...
                {
                    System.out.println(threadName + " **** EXCEPTION on connection  "); 
                    e.toString();
            ...
            {    
                try{
                    System.out.println(threadName + " **** Creating 1st JDBC Statement object");    
                    stmt = con.createStatement();                                            
                    System.out.println(threadName + " **** Created 1st JDBC Statement object");    
                }
                catch (Exception e)
                {
                    System.out.println(threadName + " **** EXCEPTION in Created 1st JDBC Statement object");    
                   // e.getMessage();
                    e.printStackTrace();
                ...
                catch (InterruptedException e)
                {
                  ...
                  /* rs = stmt.executeQuery(" SELECT  AUTHID, count FROM SYSIBMADM.APPLICATIONS  WHERE DB_NAME LIKE '%TI%'\n" +
                " AND AUTHID LIKE '%ZONE%' OR DB_NAME LIKE '%TI%' AND authid LIKE '%GLOBAL%'\n" +
                " OR DB_NAME LIKE '%TI%' AND authid LIKE 'TRADE%'\n" +
                " GROUP BY AUTHID");*/


                    String sql = "SELECT DEPLOYMENT_ID, SOFTWARE_VERSION, CONNECTION_STATUS, LAST_UPDATED from " + schema + ".STS_DEPLOYMENT_SERVER";
                    ...
                    if (dbType.equals("OracleForTI25") ||  dbType.equals("DB2ForTI229"))
                    {
                        returnvalue = new result(true, "&nbsp; &nbsp; database up","");
                        return returnvalue;
                    }
                    System.out.println(threadName + " **** Executing sql statement 1st time ");    
                    try
                    {
                    rs1 = stmt.executeQuery(sql);
                    System.out.println(threadName + " **** Executed sql statement 1st time ");  
                    doFirstPass =true;
                    ...
                    catch (InterruptedException e)
                    {
                     ...
                     if (doFirstPass)
                    {     
                        System.out.println(threadName + " **** starting first pass ");    
                       //----------------  first pass -------------------------
                       try
                       {
                            while ( rs1.next()) 
                            {
                                ...
                                System.out.println(threadName + " **** inside first pass ");    
                                ...
                                if (rs1.getString(3).equals("CONNECTED"))
                                {
                                     ...
                                     System.out.println(threadName + " **** someuthing's up ");    
                                    if (rs1.getString(1).equals("TI1"))
                                    {
                                         System.out.println(threadName + " **** TI1's up ");    

                                       //answer =   "ZONE1: " + rs1.getString(2);  // /// if we have a global and several  zone, then we potentaillay have more than 3 versions to report
                                      /* rslt1.vers = "ZONE1: " + rs1.getString(2);
                                       rslt1.timeStamp = rs1.getString(3);
                                       rslt1.up = true;
                                       answers[0] =  rslt1;*/
                                        ...
                                        */
                                        //answers[0].populate( true, "ZONE1: " + rs1.getString(2) +  "&nbsp;",  rs1.getString(4));
                                        answers[0] = new result( true, "ZONE1: " + rs1.getString(2) +  "&nbsp;",  rs1.getString(4));
                                    ...
                                    else
                                    {
                                        if (rs1.getString(1).equals("TI2"))
                                             {
                                                 System.out.println(threadName + " **** TI2's up ");    

                                                //answer =  rs1.getString(2);  // /// if we have a global and several  zone, then we potentaillay have more than 3 versions to report
                                                /*rslt1.vers =   "ZONE2: " + rs1.getString(2); 
                                                rslt1.timeStamp = rs1.getString(3);
                                                rslt1.up = true;*/
                                             /*
                                                rslt1.up = true;
                                                ...
                                                rslt1.timeStamp = rs1.getString(4);
                                                */
                                                //answers[1].populate(true , "ZONE2: " + rs1.getString(2) +  "&nbsp;", rs1.getString(4));
                                                answers[1]= new result(true , "ZONE2: " + rs1.getString(2) +  "&nbsp;", rs1.getString(4));

                                             }
                                             else  //GLOBAL    ???? and any  3rd zone ???
                                            {
                                            System.out.println(threadName + " **** GLOBAL's up ");    

                                             //answer = "GLOBAL: " +  rs1.getString(2);  // /// if we have a global and several  zone, then we potentaillay have more than 3 versions to report
                                            /* rslt1.vers = "GLOBAL: " +  rs1.getString(2);
                                             rslt1.timeStamp = rs1.getString(4);
                                             rslt1.up = true;*/
                                             //answers[2] =  rslt1;
                                             /*rslt1.vers = "GLOBAL: " +  rs1.getString(2) +  "&nbsp;";
                                             ...
                                             rslt1.timeStamp = rs1.getString(4);
                                            */
                                            // answers[2].populate(true, "GLOBAL: " +  rs1.getString(2) +  "&nbsp;", rs1.getString(4));
                                              answers[2]= new result(true, "GLOBAL: " +  rs1.getString(2) +  "&nbsp;", rs1.getString(4));
                                ...
                                //answer = answer + " " + rs.getString(1) + rs.getString(2) + rs.getString(3) + rs.getString(4); 
                            }
                       }
                       catch (Exception e)
                       {
                          System.out.println(threadName + " **** EXCEPTION in 1st pass ");
                          //e.toString();
                           e.printStackTrace();
                       ...
                       {
                           System.out.println(threadName + " **** finished first pass successfully");
                       }
                        else
                        {   System.out.println(threadName + " **** finished first pass UNsuccessfully");
                            returnvalue = new result(false, "&nbsp; &nbsp; system down","");
                            return returnvalue; 
                        }

                       // now wait 12 s. and do it again to check if the timestamps ave changed
                       ...
                       if (answers[0].isup() ||  answers[1].isup() || answers[2].isup())
                       {
                         ...
                         /*  con2 = DriverManager.getConnection (connectionString, user, password);
                           System.out.println(threadName + " **** Got 2nd connection");
                           */
                            stmt2 = con.createStatement();                                            
                           System.out.println(threadName + " **** Created 2nd JDBC Statement object");   
                           try
                           {
                               sleep(100);
                           }
                           catch (InterruptedException e)
                           {
                            ...
                            System.out.println(threadName + " **** Executing sql statement 2nd time ");    
                           ...
                           catch (InterruptedException e)
                           {
                            ...
                            System.out.println(threadName + " **** starting second pass ");    
                           while (  rs2.next()) 
                           {
                                System.out.println(threadName + " **** inside second pass ");  
                               try
                               {
                                   sleep(100);
                               }
                               ...
                               if (rs2.getString(1).equals("TI1")   ) 
                               { // no need to check if it was down on 1st pass
                                System.out.println(threadName + " **** checking for TI1 (2)");

                                  if(!rs2.getString(4).equals(answers[0].timeStamp) &&   answers[0].isup())
                                  {
                                      answers[0]= new result(true, answers[0].vers, answers[0].timeStamp);
                                      System.out.println(threadName + " **** TI1's up ");  
                                  }
                                  else
                                  {
                                      answers[0] = new result(false, answers[0].vers, answers[0].timeStamp);
                                      System.out.println(threadName + " **** TI1's down ");    
                                  }
                               }
                               else
                               {
                                   if (rs2.getString(1).equals("TI2")   )
                                   {// no need to check if it was down on 1st pass
                                       System.out.println(threadName + " **** checking for TI2 (2)");

                                       if(!rs2.getString(4).equals(answers[1].timeStamp) &&   answers[1].up == true)
                                       {
                                           answers[1] =  new result(true, answers[1].vers, answers[1].timeStamp);
                                           System.out.println(threadName + " **** TI2's up (2)");  
                                       }
                                       else
                                       {
                                           answers[1] = new result(false, answers[1].vers, answers[1].timeStamp);
                                           System.out.println(threadName + " **** TI2's down (2)");   
                                       }
                                   }
                                   else  //GLOBAL and any  3rd zone....
                                   {
                                        System.out.println(threadName + " **** checking for GLOBAL (2)");

                                       if(!rs2.getString(4).equals(answers[2].timeStamp)  &&   answers[2].up == true)
                                      {// no need to check if it was down on 1st pass
                                          answers[2] = new result(true, answers[2].vers, answers[2].timeStamp);
                                          System.out.println(threadName + " **** GLOBAL's up (2)");  
                                      }
                                      else
                                      {
                                          answers[2] = new result(false, answers[2].vers, answers[2].timeStamp);
                                          System.out.println(threadName + " **** GLOBAL's down (2)");    
                                      }
               ...
               {
                    //returnvalue = new result(areweup ,"(last time up: &nbsp; &nbsp;" +answers[2].timeStamp +") &nbsp; " + answers[0].vers + answers[1].vers + answers[2].vers , "" );
                   /*
                   String tst =  answers[0].vers + answers[1].vers + answers[2].vers;
                   if(! tst.isEmpty())
                   {*/
                        returnvalue = new result(areweup , "&nbsp; &nbsp; system down <br>" + answers[0].vers + answers[1].vers + answers[2].vers , "" );
                  /* }     
                   else
                   {
                        returnvalue = new result(areweup , "&nbsp; &nbsp; system down &nbsp; &nbsp;" + answers[0].vers + answers[1].vers + answers[2].vers , "" );
                   }*/
               }
...
/*
      ...
      returnvalue.up= true;  // true / false
// amd from there we finish the xml tag


/// ?   1 thread per tag?
// so i read my xml file and in a loop i start as many threads as elemest in my xl file
// and  in a locked object I pass back the bit of string about that element that i just finished
